...
main(int argc, char ** argv)
{
    if (argc < 4)
    {
        ...
        std::cerr << argv[0] << " inputImageFile outputImageFile MaxWindowRadio" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    // Software Guide : BeginCodeSnippet
    int maxRadio = atoi(argv[3]);
    NeighborhoodIteratorType::RadiusType radius;
    radius.Fill(maxRadio);
    ...
    {
        int currentWindowRadius = 1;
        it.GetIndex();
        PixelType zXY = it.GetCenterPixel();
        PixelType zMed, zMin, zMax;

        while(currentWindowRadius <= maxRadio)
        {
            //Get Dynamic Window
            std::vector<NeighborhoodIteratorType::OffsetType> neighborhood = getDynamicNeighborhood(currentWindowRadius);
            //Get Dynamic Array for that neighborhood
            std::vector<PixelType> pixelValues = getPixelValuesFromNeighborhood(neighborhood, it);

            //set Zmin, zMax and Zmed
            setZValues(pixelValues, zMed, zMin, zMax);

            //SetL11, and L12
            PixelType l11 = zMed - zMin;
            PixelType l12 = zMed - zMax;

            if(l11 > 0 && l12 < 0)
            {
                if(zMed != zXY) {
                    PixelType l21 = zXY - zMin;
                    PixelType l22 = zXY - zMax; /// in the article this is zMin, however it would make no sense given that this would be equal to l21
                    if (l21 > 0 && l22 < 0) {
                        out.Set(zXY);
                        break;
                    }
            ...
            {
                currentWindowRadius++;
                if(currentWindowRadius > maxRadio)
                {
                    out.Set(zXY);
                }
            }
